...
LEFT JOIN ' + quotename(@databasename) +  N'.sys.dm_db_index_usage_stats AS stat ON 
    stat.database_id = DB_ID() 
    and si.object_id=stat.object_id 
    and si.index_id=stat.index_id
LEFT JOIN ' + quotename(@databasename) +  N'.sys.partition_schemes AS psc ON si.data_space_id=psc.data_space_id
LEFT JOIN ' + quotename(@databasename) +  N'.sys.partition_functions AS pf ON psc.function_id=pf.function_id
LEFT JOIN ' + quotename(@databasename) +  N'.sys.filegroups AS fg ON si.data_space_id=fg.data_space_id
...
/* Partitions */ OUTER APPLY ( 
    ...
    JOIN ' + quotename(@databasename) +  N'.sys.dm_db_partition_stats AS ps ON
...
EXEC sp_executesql @stmt = @dsql, @params = N'@tablename sysname', @tablename=@tablename;
